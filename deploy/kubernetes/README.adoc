...
`jq` is useful for extracting data from `kubecl` output, it can be downloaded from https://stedolan.github.io/jq/download/
...
Type:                     LoadBalancer
IP:                       10.47.247.7
LoadBalancer Ingress:     {{EXTERNAL_IP}}
Port:                     <unset>  80/TCP
TargetPort:               8080/TCP
...
----
$ curl http://{{EXTERNAL_IP}}/v2/catalog -u admin:supersecret
...
Note the value for `LoadBalancer Ingress` column and `Port` in the output above.
...
Create a Kubernetes secret to store these credentials (the username and password values in the deployment file are base64 encoded):
