...
NOTE: This step will deploy the `webmvc` variation of the sample service broker.
To deploy the `weflux` variation of the sample service broker, modify the path in the link:manifest.yml[manifest.yml] file.
...
+ name:       bookstore-service-broker
  path:       build/libs/bookstore-service-broker-webmvc-0.0.1.BUILD-SNAPSHOT.jar
+ memory:     1G
  routes:
